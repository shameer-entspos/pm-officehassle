...
interface MultiSelectContextProps {
  ...
  ref: React.RefObject<HTMLInputElement | null>; // Updated to allow null
...
>(({ className, children, ...props }, ref) => {
  const { value, activeIndex } = useMultiSelect();
  ...
  return (
    <div
      ref={ref}
      className={cn(
        'ring-muted border-input bg-popover flex h-10 w-full flex-wrap gap-1 rounded-md border p-1 text-sm ring-1',
        {
    ...
    >
      {value.length > 0 && (
        <Badge
          className={cn(
            '!bg-background flex items-center gap-1 !p-[4px] !px-2'
          )}
          variant={'secondary'}
        >
